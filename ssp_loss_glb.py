...
def cal_damage_area(start_string):
    ...
    ds     = xr.open_dataset(f'{dir_mod_adj}/{ssp}/{fname}')
    ds_org = xr.open_dataset(f'{dir_mod_org}/{ssp}/{fname}')
...
def process_each_model(start_string,price_level):
    ...
    if np.isnan(data_damage_area).all():
        ...
        print(f'Processing {v_crop}-{ssp}-{start_string}...')
        ...
        data_loss_usd.to_csv(f'{f_loss}/loss_usd/loss_usd_{v_crop}_{start_string}_{ssp}.csv', index=False)
        data_loss_production.to_csv(f'{f_loss}/loss_production/loss_production_{v_crop}_{start_string}_{ssp}.csv', index=False)
...
if __name__ == '__main__':
    ...
    #====== dir for crop yield
    dir_mod_adj = f'../output/anomaly_mod/isimip_adj'
    dir_mod_org = f'../output/anomaly_mod/isimip_org'
